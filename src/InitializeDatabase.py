...
def create_indexes():
    queries = [
        # Index Bitmap
        "CREATE NONCLUSTERED INDEX IX_User_isActive ON [User](isActive);",
        "CREATE NONCLUSTERED INDEX IX_Klien_isActive ON Klien(isActive);",
        "CREATE NONCLUSTERED INDEX IX_Event_status ON Event(status_event);",
        "CREATE NONCLUSTERED INDEX IX_Vendor_isActive ON Vendor(isActive);",

        # Index B-tree
        "CREATE UNIQUE NONCLUSTERED INDEX IX_User_username ON [User](username);",
        "CREATE NONCLUSTERED INDEX IX_Event_tanggal ON Event(tanggal_event);",

        # Index Composite
        "CREATE NONCLUSTERED INDEX IX_User_role_active ON [User](role, isActive);",
        "CREATE NONCLUSTERED INDEX IX_Vendor_active_jenis ON Vendor(isActive, id_JenisVendor);",
        "CREATE NONCLUSTERED INDEX IX_Klien_active_nama ON Klien(isActive, nama);",

        # Index Foreign Key
        "CREATE NONCLUSTERED INDEX IX_Event_klien_fk ON Event(id_Klien);",
        "CREATE NONCLUSTERED INDEX IX_EventVendor_event_fk ON EventVendor(id_Event);",
        "CREATE NONCLUSTERED INDEX IX_UserEvent_event_fk ON UserEvent(id_Event);",
        "CREATE NONCLUSTERED INDEX IX_Event_jenis_fk ON Event(id_JenisEvent);",
        "CREATE NONCLUSTERED INDEX IX_Vendor_jenis_fk ON Vendor(id_JenisVendor);"
    ]
    count = 1
    for query in queries:
        if execute_query(query):
            ...
            print("Indexing gagal.")

    return f"Indexing berhasil: {count}."
...
def insertUserEvent():
    ...
    INSERT INTO 
        UserEvent
    VALUES
        -- Event 1: Wedding Ceremony - Ditangani oleh 2 asisten
        (2, 1), -- Siti Nurhaliza (asisten01)
        (3, 1), -- Budi Santoso (asisten02)
        -- Event 2: Corporate Meeting - Ditangani oleh 2 asisten
        (4, 2), -- Linda Permata (asisten03)
        (5, 2), -- Rudi Hartono (asisten04)
        -- Event 3: Product Launch - Ditangani oleh 2 asisten
        (6, 3), -- Maya Sari (asisten05)
        (7, 3), -- Doni Pratama (asisten06)
        -- Event 4: Graduation Ceremony - Ditangani oleh 3 asisten 
        (8, 4), -- Rina Anggraini (asisten07)
        (9, 4), -- Andi Susanto (asisten08)
        (10, 4), -- Dewi Lestari (asisten09)
        -- Event 5: Birthday Party - Ditangani oleh 1 asisten 
        (11, 5), -- Hendra Wijaya (asisten10)
        -- Event 6: Workshop - Ditangani oleh 2 asisten
        (12, 6), -- Fitriani (asisten11)
        (13, 6), -- Agus Salim (asisten12)
        -- Event 7: Charity Gala - Ditangani oleh 2 asisten 
        (14, 7), -- Ratna Sari (asisten13)
        (15, 7), -- Bambang Irawan (asisten14)
        -- Event 8: Tech Conference - Ditangani oleh 3 asisten 
        (16, 8), -- Novi Rahayu (asisten15)
        (17, 8), -- Teguh Prasetyo (asisten16)
        (18, 8), -- Indira Putri (asisten17)
        -- Event 9: Anniversary - Ditangani oleh 2 asisten
        (19, 9), -- Fajar Nugroho (asisten18)
        (20, 9), -- Kartika Dewi (asisten19)
        -- Event 10: Baby Shower - Ditangani oleh 1 asisten 
        (2, 10), -- Siti Nurhaliza (asisten01)
        -- Event 11: Engagement Party - Ditangani oleh 1 asisten
        (3, 11), -- Budi Santoso (asisten02)
        -- Event 12: Medical Seminar - Ditangani oleh 2 asisten
        (4, 12), -- Linda Permata (asisten03)
        (5, 12), -- Rudi Hartono (asisten04)
        -- Event 13: Art Exhibition - Ditangani oleh 2 asisten
        (6, 13), -- Maya Sari (asisten05)
        (7, 13), -- Doni Pratama (asisten06)
        -- Event 14: Music Concert - Ditangani oleh 3 asisten 
        (8, 14), -- Rina Anggraini (asisten07)
        (9, 14), -- Andi Susanto (asisten08)
        (10, 14), -- Dewi Lestari (asisten09)
        -- Event 15: Cultural Festival - Ditangani oleh 4 asisten 
        (11, 15), -- Hendra Wijaya (asisten10)
        (12, 15), -- Fitriani (asisten11)
        (13, 15), -- Agus Salim (asisten12)
        (14, 15), -- Ratna Sari (asisten13)
        -- Event 16: Networking Event - Ditangani oleh 1 asisten
        (15, 16), -- Bambang Irawan (asisten14)
        -- Event 17: Team Building - Ditangani oleh 1 asisten
        (16, 17), -- Novi Rahayu (asisten15)
        -- Event 18: Awards Ceremony - Ditangani oleh 2 asisten
        (17, 18), -- Teguh Prasetyo (asisten16)
        (18, 18), -- Indira Putri (asisten17)
        -- Event 19: Fashion Show - Ditangani oleh 2 asisten
        (19, 19), -- Fajar Nugroho (asisten18)
        (20, 19), -- Kartika Dewi (asisten19)
        -- Event 20: School Anniversary - Ditangani oleh 2 asisten
        (2, 20), -- Siti Nurhaliza (asisten01)
        (3, 20); -- Budi Santoso (asisten02)
    '''
    return "UserEvent berhasil dimasukkan." if execute_query(query) else "UserEvent gagal dimasukkan."

def insertJenisEvent():
    query = '''
    INSERT INTO 
        JenisEvent
    VALUES 
        ...
        ('Art Gallery Opening');
    '''
    return "JenisEvent berhasil dimasukkan." if execute_query(query) else "JenisEvent gagal dimasukkan."

def insertJenisVendor():
    query = '''
    INSERT INTO 
        JenisVendor
    VALUES 
        ...
        ('Furniture Rental');
    '''
    return "JenisVendor berhasil dimasukkan." if execute_query(query) else "JenisVendor gagal dimasukkan."
