...
def lihat_rincian_budgeting(id_Event):
    query = '''
            SELECT 
                v.id_Vendor, v.nama, jenis_vendor, harga_dealing 
            FROM 
                EventVendor ev 
                JOIN Vendor v ON v.id_Vendor = ev.id_Vendor 
                JOIN JenisVendor jv ON v.id_JenisVendor = jv.id_JenisVendor 
            WHERE 
                ev.id_Event = ? 
            '''
    hasil = fetch_all(query, (id_Event,))
    ...
    if hasil:
        hasil_format = [(id_vendor, nama_vendor,jenis_vendor, format_rupiah(harga_dealing))
            for id_vendor, nama_vendor, jenis_vendor, harga_dealing in hasil]
        kolom = ['ID Vendor', 'Nama Vendor', 'Jenis Vendor', 'Harga Dealing']
        print(tabulate(hasil_format, headers=kolom, tablefmt='grid'))
        print()
    else:
        print("Tidak ada rincian budgeting ditemukan untuk klien ini.\n")
        return "Tidak ada rincian budgeting ditemukan untuk klien ini."

def total_budget_event(id_Event):
    query = '''
            SELECT 
                e.id_Event, e.nama, SUM(harga_dealing)
            FROM 
                EventVendor ev
    ...
    if hasil and hasil[0][0] is not None:
        hasil_format = [(id_ev, nama_event, format_rupiah(total)) for id_ev, nama_event, total in hasil]
        kolom = ['ID Event', 'Nama Event', 'Total Harga Dealing']
        print(tabulate(hasil_format, headers=kolom, tablefmt='grid'))
        print()
    else:
        print("Tidak ada total harga dealing untuk event ini.")
...
def tambah_harga_dealing(id_Event, id_Vendor, harga_dealing):
    query = '''INSERT INTO 
               ...
               VALUES 
                   (?, ?, ?)'''
    print("\nTambah harga dealing berhasil.\n") if execute_query(query, (id_Event, id_Vendor, harga_dealing,)) else print("\nTambah harga dealing gagal.\n")
...
def update_harga_dealing(id_Event, id_Vendor, harga_dealing):
    ...
    '''
    print("\nUpdate harga dealing berhasil.") if execute_query(query, (harga_dealing, id_Event, id_Vendor,)) else print("\nUpdate harga dealing gagal.")
